...
*/
{
  if (is_void(zoom_factor))
    zf= 1.5;      //Default zoom factor
  else
    zf= zoom_factor;
  ...
  while (cont) {
    ...
    if (button == 1) {           //Left button   ZOOM IN
      ...
      */
      ...
      if (nlim(1) != lim(1))
        nlim(2)= mx+(lim(2)-mx)/zf;
      nlim(3)= my-(my-lim(3))/zf;
      if (nlim(3) != lim(3))
        nlim(4)= my+(lim(4)-my)/zf;
    ...
    if (!is_void(all))
      return(event);       //Return every event.
  }
}
