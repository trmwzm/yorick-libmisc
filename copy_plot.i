...
/* DOCUMENT copy_win, window_source, window_target, lmt=, clear=, pal=
   Copy the style and the graphic elements of window WINDOW_SOURCE to
   window WINDOW_TARGET
   KEYWORDS:
   lmt=   if set, copy also the limits (the default)
...
*/
{
  ...
  if (is_void(clear) || clear==1)
    fma;
  if ((is_void(pal) || pal==1) && is_array(red))
...
/* DOCUMENT replot_all (window_source, window_target, lmt=)
...
*/
{
  ...
  if (nbsin!=nbsout)
    error,"The two windows have not the same number of systems !!";
...
/* DOCUMENT replot_one_sys, window_source, sys_source, window_target, system_target, lmt=
...
func save_plot (wsrc, pal=, pdb=)
/* DOCUMENT save_plot, win, pdb=[createb(filename)];
   Save Yorick plot of window win in file 'filename'.
   The plot can be reload with load_plot.

   KEYWORDS:
   pal= : save the palette if any in the file (the default)
   ...
   ---------- save to file -----------
   save_plot,0,pdb=createb("rand_array.gdb");
   load_plot,openb("rand_array.gdb"),1;
    ---------- save to object -----------
   o= save_plot(0);
   load_plot,o,1;
 ...
 */
{
  ...
  window, wsrc;
  get_style, a,b,c,d;
  save, ostrm,"getstyle_p1",a;
  save, ostrm,"getstyle_p2",b;
  save, ostrm,"getstyle_p3",c;
  save, ostrm,"getstyle_p4",d;

  palette, rp,gp,bp,query=1;
  if (!is_void(rp) && (is_void(pal) || pal==1)) {
    rgb_pal= long(rp)+(long(gp)<<8)+(long(bp)<<16);
    save, ostrm,"palette",rgb_pal;
  }
  ...
  for (i=0;i<=nbsys;i++) {
    ...
    lmt= limits();
    nbobj= numberof(plq());
    save, ostrm,swrite(format="system_%d",i),i;
    save, ostrm,swrite(format="limits_%d",i),lmt;
    for(j=1;j<=nbobj;j++) {
      prop= plq(j);
      decomp_prop, prop,p1,p2,p3,p4,p5;
      save, ostrm,swrite(format="prop1_%d_%d",i,j),(is_void(p1)?"dummy":p1);
      save, ostrm,swrite(format="prop2_%d_%d",i,j),(is_void(p2)?"dummy":p2);
      save, ostrm,swrite(format="prop3_%d_%d",i,j),(is_void(p3)?"dummy":p3);
      save, ostrm,swrite(format="prop4_%d_%d",i,j),(is_void(p4)?"dummy":p4);
      rslt= reshape_prop(prop);
      save, ostrm,swrite(format="prop5_%d_%d",i,j),(is_void(rslt)?"dummy":rslt);
    }
  }
...
func load_plot (ob, wout, clear=, lmt=, pal=)
/* DOCUMENT load_plot, ob, wout, clear=, lmt=, pal=;
   Load Yorick plot form file 'filename' in window wout
   The plot have to be saved with save_plot.

   EXAMPLE: see save_plot

   KEYWORDS:
   lmt=   if set (default), restore also the limits
   clear= if set (default) erase the window  before loading
...
*/
{
  ...
  if (is_obj(ob)>0)
    ...
    error,"only object, or bin stream (openb).";

  if (is_obj(wout)>0)
    error,"valid inputs: > load_plot,obj,[win];";
  ...
  window, wout;
  set_style, ostrm.getstyle_p1,ostrm.getstyle_p2,ostrm.getstyle_p3,ostrm.getstyle_p4;

  if (clear)
    fma;
  if (is_obj(ostrm)==1)
    names= *get_vars(ostrm)(1);
  else
    names= ostrm(*,);
  ...
  while (++idx<=nnames) {
    if (strmatch(names(idx),"system_")) {
      plsys,get_member(ostrm,names(idx));
      limits;
      continue;
    }
    if (strmatch(names(idx),"limits_")) {
      if (lmt) limits,get_member(ostrm,names(idx));
      continue;
    }

    if (strmatch(names(idx),"prop1_")) {
      p1= get_member(ostrm,names(idx));
      p2= get_member(ostrm,names(++idx));
      p3= get_member(ostrm,names(++idx));
      p4= get_member(ostrm,names(++idx));
      rslt= get_member(ostrm,names(++idx));
...
*/
{
  ...
  if (aig==1)      return reshape_plg(prop);
    ...
    error,"Unknown graphical element !!";
}
...
/* DOCUMENT set_selected_system (saved_sys)
   ...
   also returns get_selected_system, if called as function
