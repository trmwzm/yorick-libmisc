 ...
 *  $Id$
 *  $Log$
 *  Revision 1.2  2011/04/29 22:21:24  trm
 *  change interface
 *  
 *  Revision 1.1  2010/12/15 04:23:47  trm
 *  rename conflict with yorick-z poly.i
 *
 *  Revision 1.2  2010/12/15 04:10:31  trm
 *
         ...
         for degree 4). */
{
  ...
  if (n==15) {
    ...
    if (is_void(eps)) eps= 1e-4;
    eps+= [0,0];
    drv(..,1)= (poly2(x+eps(1)/2,y,c)-poly2(x-eps(1)/2,y,c))/eps(1);
    drv(..,2)= (poly2(x,y+eps(2)/2,c)-poly2(x,y-eps(2)/2,c))/eps(2);
  }
  return drv;
}
...
/*---------------------------------------------------------------------------*/
func poly1_gcv_fit(y, x, w, &fitdat, degree=, verbose=, all=)
   ...
   SEE ALSO: poly1_fit. */
{
  ...
  fitdat= array(double, 2, degree_max-degree_min+1);
  fitdat(1,)= indgen(degree_min:degree_max);
  ...
  for (degree=degree_min ; degree<=degree_max ; degree++) {
    ...
    fitdat(2, degree-degree_min+1)= gcv_rms;
